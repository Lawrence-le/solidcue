...
async def test_discovery_node_extracts_paths_from_llm(monkeypatch) -> None:
    monkeypatch.setattr(discovery_module, "_load_discovery_cache", lambda _agent_key: None)
    monkeypatch.setattr(discovery_module, "_save_discovery_cache", lambda _agent_key, _data: None)
    monkeypatch.setattr(
        discovery_module,
        "load_agent_skill",
...
@pytest.mark.asyncio
async def test_discovery_node_uses_cache_when_present(monkeypatch) -> None:
    cached = {
        "source_paths": ["resume_agent/source/master"],
        "output_paths": ["resume_agent/generated_resumes/"],
        "source_filenames": ["ai_job_tracker"],
        "output_filenames": ["YYYYMMDD_resume"],
    }
    monkeypatch.setattr(discovery_module, "_load_discovery_cache", lambda _agent_key: cached)
    ...
    monkeypatch.setattr(discovery_module, "_extract_paths_with_llm", _fake_llm)

    result = await discovery_node({"agent_key": "resume_builder"})

    assert not llm_called["called"]
    assert result["source_paths"] == ["resume_agent/source/master"]
    assert result["source_filenames"] == ["ai_job_tracker"]
    assert result["output_filenames"] == ["YYYYMMDD_resume"]
